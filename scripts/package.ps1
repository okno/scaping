...
foreach ($document in @('DEPENDENCIES.md','SECURITY.md','VERIFICATION.md')) {
...
$archive = Join-Path $dist 'scaping-1.0.0-windows-x64.zip'
Compress-Archive -Path (Join-Path $stage '*') -DestinationPath $archive -Force
Write-Host "Pacchetto: $archive"
Write-Host "SHA256 eseguibile: $($hash.Hash)"
