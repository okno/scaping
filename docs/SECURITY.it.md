...
[English](SECURITY.md) · [Italiano](SECURITY.it.md) · [README](../README.it.md)

## Resident e input

Il manifest richiede `asInvoker`. La UI e il ping non richiedono amministratore. L'IP è un singolo IPv4 numerico validato; non sono ammessi host, subnet, liste o comandi. Target e lingua di ogni scansione sono copie della configurazione al momento dell'avvio.
...
## Configurazione e lingua

Lo schema di configurazione versione 2 salva la lingua come `it` oppure `en`. I file versione 1 vengono caricati in italiano, preservando target e altre impostazioni. La migrazione avviene in memoria; il successivo salvataggio atomico riuscito scrive la versione 2. Cambiare lingua non seleziona un nuovo target.

Nello stato iniziale non configurato è possibile salvare la lingua con IP vuoto senza generare traffico. Dopo aver configurato un target, applicare le impostazioni richiede un IPv4 valido. Una scansione in corso conserva IP e lingua originali: il cambio della lingua dell'interfaccia non modifica il lavoro e non riscrive l'output già raccolto. Token di protocollo, argomenti dei processi, dati XML, banner e output degli strumenti esterni non vengono tradotti.

## Worker UAC temporaneo

Il worker esiste solo per una fase che richiede privilegi. Accetta PID/tempo di creazione del padre e nonce; dal canale riceve esclusivamente una richiesta versionata con IPv4, identificatore di lingua validato e profilo fisso. Non riceve percorsi eseguibili, percorsi di scrittura o argomenti liberi.
